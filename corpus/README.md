...
## Every reader, not one: `cmd/judges`

The smallest PDF is worth nothing if a reader in the field disagrees with the
others about it. `cmd/judges` runs every reference consumer present on the
machine over the corpus PDFs — html2pdf's outputs and, as the control,
Chrome's PDFs of the same pages — and writes `JUDGES.md` + `judges.json`:
...
| pdf.js (`judges/pdfjs-*.mjs` under node) | Firefox's engine | pages, text, render |
...
cd .. && go run ./cmd/judges      # add -pdfium /path/to/pdfium_test when built
```
